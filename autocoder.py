...
- Use python's datetime module to get the current date and time.
- Use python's AppOpener module to open and close app
- Use python's pyautogui to control mouse and keyboard (youtube, browser control using shortcuts)
...
"""
    ...
    def main(self, response: str):
        ...
        if "CONTINUE" in response or not self.internal_exec:
            ...
            default_path = os.path.join(os.path.dirname(__file__), ".cache", "webscout")
            os.makedirs(default_path, exist_ok=True)
            path_to_script = self.path_to_script if self.path_to_script else os.path.join(default_path, "execute_this.py")
